...
test('Discord worker fast mode and image timeout are environment driven', () => {
  ...
  assert.match(ecosystemSource, /DISCORD_SEND_TIMEOUT_SECONDS: "20"/)
  assert.match(ecosystemSource, /DISCORD_BOUND_CHANNEL_CACHE_TTL_SECONDS: "5"/)
  assert.match(ecosystemSource, /DISCORD_MESSAGE_IMAGE_REPLY_TIMEOUT_SECONDS: "90"/)
  ...
  assert.match(ecosystemSource, /KEYWORD_TEXT_SEARCH_API_QUEUE_TIMEOUT_SECONDS: "10"/)
...
test('Discord workers use the bounded production shard and gateway recovery policy', () => {
  const workerSource = readSource('backend/bot_worker.py')
  const ecosystemSource = readSource('ecosystem.config.js')

  assert.match(ecosystemSource, /process\.env\.BOT_WORKER_COUNT \|\| process\.env\.BOT_SHARD_COUNT \|\| 4/)
  assert.match(ecosystemSource, /KEYWORD_TEXT_SEARCH_MAX_INFLIGHT: "1"/)
  assert.match(ecosystemSource, /DISCORD_DISCONNECT_OFFLINE_GRACE_SECONDS: "300"/)
  assert.match(ecosystemSource, /BOT_WATCHDOG_DISCONNECTED_GRACE_SECONDS: "600"/)
