...
test('Discord worker fast mode and image timeout are environment driven', () => {
  ...
  assert.match(ecosystemSource, /DISCORD_MESSAGE_IMAGE_REPLY_TIMEOUT_SECONDS: "720"/)
  assert.match(ecosystemSource, /DISCORD_IMAGE_REPLY_MAX_ATTACHMENTS_PER_MESSAGE: "8"/)
  assert.match(ecosystemSource, /LIVE_IMAGE_SEARCH_QUEUE_MAX_SIZE: "1024"/)
  assert.match(ecosystemSource, /KEYWORD_TEXT_SEARCH_API_QUEUE_TIMEOUT_SECONDS: "300\.0"/)
})
