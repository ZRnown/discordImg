...
test('app shell exposes keyword image review entry', () => {
  const pageSource = readFileSync(new URL('./components/app-page-client.tsx', import.meta.url), 'utf8')
  const sidebarSource = readFileSync(new URL('./components/app-sidebar.tsx', import.meta.url), 'utf8')

  assert.equal(pageSource.includes('KeywordImageReviewView'), true)
  assert.equal(sidebarSource.includes('keyword-image-review'), true)
})
