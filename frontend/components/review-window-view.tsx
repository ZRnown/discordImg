...
export function ReviewWindowView({ isActive = true }: { isActive?: boolean }) {
  ...
  const fetchWebsites = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/websites", { credentials: "include", cache: "no-store" })
  ...
  const fetchReviewBarkSettings = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/user/settings", { credentials: "include", cache: "no-store" })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(getApiErrorMessage(data, "获取审核通知设置失败"))
      }
      ...
      toast.error(getApiErrorMessage(error, "获取审核通知设置失败"))
    } finally {
  ...
  const fetchItems = useCallback(async (options?: { preserveSelection?: boolean; silent?: boolean }) => {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(getApiErrorMessage(data, "获取待审列表失败"))
      }
      ...
      if (!options?.silent) {
        toast.error(getApiErrorMessage(error, "获取待审列表失败"))
      }
    } finally {
  ...
  const saveReviewBarkSettings = async (sourceSettings = reviewBarkSettings, options?: { silent?: boolean }) => {
    if (!sourceSettings.bark_device_key.trim()) {
    ...
    try {
      const response = await fetch("/api/user/settings", {
        ...
        credentials: "include",
        body: JSON.stringify({
          bark_server_url: sourceSettings.bark_server_url,
          bark_device_key: sourceSettings.bark_device_key,
          review_bark_enabled: sourceSettings.review_bark_enabled,
          review_bark_mode: sourceSettings.review_bark_mode,
          review_bark_count_threshold: sourceSettings.review_bark_count_threshold,
          review_bark_interval_minutes: sourceSettings.review_bark_interval_minutes,
        }),
      })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(getApiErrorMessage(data, "保存审核通知设置失败"))
      }
      return true
    } catch (error) {
      if (!options?.silent) {
        toast.error(getApiErrorMessage(error, "保存审核通知设置失败"))
      }
      return false
    } finally {
      if (!options?.silent) {
  ...
  const testReviewBarkNotification = async () => {
    ...
    try {
      const saved = await saveReviewBarkSettings(reviewBarkSettings, { silent: true })
      if (!saved) return
      ...
      const response = await fetch("/api/user/bark-test", {
        ...
        credentials: "include",
        body: JSON.stringify({
          bark_server_url: reviewBarkSettings.bark_server_url,
          bark_device_key: reviewBarkSettings.bark_device_key,
        }),
      })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(getApiErrorMessage(data, "测试推送失败"))
      }
      if (data?.device_key_updated) {
        const nextSettings = {
          ...reviewBarkSettings,
          bark_device_key: data.device_key_updated,
        }
        setReviewBarkSettings(nextSettings)
        await saveReviewBarkSettings(nextSettings, { silent: true })
      }
      ...
      toast.error(getApiErrorMessage(error, "测试推送失败"))
    } finally {
        ...
        <CardContent className="space-y-4 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <h2 className="text-lg font-semibold">审核窗口</h2>
              <p className="text-sm text-muted-foreground">
                这里展示待人工审核的关键词回复，并支持 Bark 通知。
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="review-bark-device-key">Bark 设备 Key</Label>
              <Input
                id="review-bark-device-key"
                type="password"
                value={reviewBarkSettings.bark_device_key}
                onChange={(event) => setReviewBarkSettings(prev => ({ ...prev, bark_device_key: event.target.value }))}
                placeholder="请输入 Bark 设备 Key"
              />
            </div>
            <div className="space-y-1">
              <Label htmlFor="review-bark-server-url">Bark 服务地址</Label>
              <Input
                id="review-bark-server-url"
                value={reviewBarkSettings.bark_server_url}
                onChange={(event) => setReviewBarkSettings(prev => ({ ...prev, bark_server_url: event.target.value }))}
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <Label htmlFor="review-bark-enabled" className="text-sm font-medium">审核 Bark 通知</Label>
              <p className="text-xs text-muted-foreground mt-1">
                开启后会按当前账号的 Bark 配置推送待审提醒。
              </p>
            </div>
            <Switch
              id="review-bark-enabled"
              checked={reviewBarkSettings.review_bark_enabled}
              onCheckedChange={(checked) => setReviewBarkSettings(prev => ({ ...prev, review_bark_enabled: checked }))}
            />
          </div>

          <div className="grid gap-3 sm:grid-cols-[240px_auto] sm:items-end">
            <div className="space-y-1">
              <Label htmlFor="review-bark-mode">通知模式</Label>
              <Select
                value={reviewBarkSettings.review_bark_mode}
                onValueChange={(value) => setReviewBarkSettings(prev => ({
                  ...prev,
                  review_bark_mode: value === "interval" ? "interval" : "count",
                }))}
              >
                <SelectTrigger id="review-bark-mode">
            ...
            {reviewBarkSettings.review_bark_mode === "count" ? (
              <div className="space-y-1">
                <Label htmlFor="review-bark-count-threshold">待审达到</Label>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onChange={(event) => setReviewBarkSettings(prev => ({
                      ...prev,
                      review_bark_count_threshold: normalizePositiveInteger(event.target.value, 5),
                    }))}
                    className="w-28"
                  />
                  <span className="text-xs text-muted-foreground">条时提醒一次</span>
                </div>
              </div>
            ) : (
              <div className="space-y-1">
                <Label htmlFor="review-bark-interval-minutes">通知间隔</Label>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onChange={(event) => setReviewBarkSettings(prev => ({
                      ...prev,
                      review_bark_interval_minutes: normalizePositiveInteger(event.target.value, 60),
                    }))}
                    className="w-28"
                  />
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button type="button" variant="outline" onClick={() => void testReviewBarkNotification()} disabled={testingReviewBark}>
                {testingReviewBark ? "测试推送中..." : "发送测试推送"}
              </Button>
              <Button type="button" onClick={() => void saveReviewBarkSettings()} disabled={savingReviewBarkSettings}>
      ...
      <Card>
        <CardContent className="space-y-4 p-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2 lg:flex-col lg:items-stretch">
                      <Button size="sm" onClick={() => void submitAction("approved", [item.id])} disabled={actionInFlight !== null}>
                        <CheckCircle2 className="mr-2 size-4" />
                        通过
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => void submitAction("rejected", [item.id])} disabled={actionInFlight !== null}>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div>{selectedCount > 0 ? `当前选中 ${selectedCount} 条` : "可直接批量处理"}</div>
          </div>
        </CardContent>
      </Card>
    </div>
