...
const operationSteps: TutorialStep[] = [
  ...
  },
  {
    ...
    view: "accounts",
    title: "账号页先定全局默认值",
    description: "进入账号与规则页后，先看这里的全局设置。相似度阈值和回复延迟都会先用这里的默认值，网站级设置只有在你单独填写时才覆盖它。像关键词命中上限这类更细的拦截条件，现在放到下面的消息过滤规则里单独添加。",
  ...
  },
  {
    ...
    view: "accounts",
    title: "关键词命中上限现在从过滤规则里单独添加",
    description: "在“消息过滤”里新增一条“关键词命中上限”规则，就能控制一条消息里命中多少个不同关键词后直接不回复。比如你设成 2，消息里同时出现 B30、B22、B44，这条就会被拦掉。它按不同关键词数量算，不是按同一个词重复次数算。",
    selector: "[data-tutorial='accounts-message-filters']",
    label: "关键词上限",
    alert: "这条规则已经移到过滤规则体系里，和身份组屏蔽、用户重复发送放在一起管理。",
    shape: "ring",
    tone: "amber",
    placement: "top",
  },
...
const ruleDetailSteps: TutorialStep[] = [
  ...
  },
  {
    ...
    view: "accounts",
    title: "网站级关键词上限也放进网站自己的过滤规则里",
    description: "如果某个网站特别容易被多关键词消息刷屏，就在这个网站自己的“消息过滤”里再加一条“关键词命中上限”。这样它只影响当前网站，不会把别的网站一起收紧。",
    selector: "[data-tutorial='accounts-website-filters']",
    label: "单站上限",
    alert: "网站过滤规则是单站生效，特别适合给高噪声网站单独加严。",
    shape: "ring",
    tone: "amber",
    placement: "top",
  },
  {
    ...
    description: "这一块是整套系统最细的规则层。常见用法包括：拦包含某段文本的消息、拦开头或结尾固定格式、用正则拦特定模式、拦图片消息、拦某个用户或身份组、拦参考图相似的噪声图、拦数字超范围的尺码消息、拦同一用户短时间重复发送，也能单独添加“关键词命中上限”。身份组ID和用户ID都支持一次填多个，直接用英文逗号、中文逗号或换行分开就行。",
    selector: "[data-tutorial='accounts-message-filters']",
    label: "过滤规则",
